...
class Custom_Walker_Category extends Walker_Category {
	...
	public $collapsed = ['', 'true','show'];
	// public $collapsed = ['collapsed', 'false',''];
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
        ...
        if ( $depth === 0 ) {
            ...
            $link = '<button class="btn btn-toggle d-inline-flex align-items-center border-0 text-uppercase text-start ps-0 fs-6 '.$this->collapsed[0].'" data-bs-toggle="collapse" data-bs-target="#'.$category->slug.'-collapse" aria-expanded="'.$this->collapsed[1].'">'.$cat_name.'</button>';
        } else {
            $link = sprintf(
                '<a%s>%s</a>',
                $attributes,
